                             ...
                             size_t len, value_t type, int weak)
{
    if (type == fl_ctx->bytesym) {
        ...
        if (nt == N_NUMTYPES) {
            ...
            static size_t (*jl_static_print)(ios_t*, void*) = NULL;
            static int init = 0;
            if (jl_atomic_load_acquire(&init) == 0) {
#if defined(RTLD_SELF)
                jl_atomic_store_release(&jl_static_print, (size_t (*)(ios_t*, void*))
...
#endif
                jl_atomic_store_release(init, 1);
            }
